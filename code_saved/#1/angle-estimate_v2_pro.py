...
###
def display_monitor(dist_m, angle_m, center_flag, center_x, center_y, arrival_flag, speed):
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        #         names[c] if opt.hide_conf else f'{x_top:.2f} {y_top:.2f} {width:.2f} {height:.2f}')
                        #     # distancei = (2 * 31.4 * 180)*16.5 / (width+height)/43
                        #     # @distancei = (2 * 31.4 * 180*10) / (width*20 + (height/6)) /0.47
                        #     distancei = (2 * 31.4 * 180) * 2.3 / (width + height)
                        #     print('=distance', distancei)
                        #     # label2 = f'{names[0]} {distancei:.1f}cm'
                        #     plot_one_box_center(xyxy, im0, label=f'{distancei:.1f}cm', color=colors(c, True),
                        #                         line_thickness=1)
                        #     # cv2.putText(frame, str(label2), (x_top,y_top+5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors(c, True),2)
                        #     if opt.save_crop:
                        #         save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)
                        # distanci = distance estimate value
                        #XXX
                        #f=open("value1.txt" as 'w')
                        if c == 1:
                            distancei = (2 * 31.4 * 180) * 9.5 / (width + height)
                            ###print(type(distancei))
                            vals=100
                            value=100
                            v=0
                            ###
                            if vals > 40:
                                ...
                                if len(s_rcv) > 8:
                                    ...
                                    # print(val2)
                                    display_monitor(val2, ang, True, 2, 0, False, v)
                                else:
                                    pass

                            else:
                                break  ## when val2<40 cm, return to main
                            ...
                            display_monitor(dist_m, angle_m, True, 2, 0, False, v)
                            f.write(dist_m)
                            f.write('\n')
                            ###
                            # draw all 3 lines
                            ...
                            display(distancei,angle_text, True, 2, 0, True, v)
                            # -------------------------------------------------------------------------------------------------
                            #print('')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='pallet0623b.pt', help='model.pt path(s)')
...
###
    ...
    angle_m=90
    dist_m=300
    ### have to select between distancei and val2
    display_monitor(dist_m, angle_m, True, 2, 0, False, v)
    ###
    ...
    ###ser.close()
    #  have to select between distancei and val2
    display_monitor(distancei, angle_m, True, 2, 0, True, v)
