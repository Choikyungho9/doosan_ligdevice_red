            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if c == 1:
                            distancei = (2 * 31.4 * 180) * 5 / (width + height)  
                            dist_i=int(distancei*10)/10
                            dist_m1=(-1.7134*math.pow(10,-22)) + (2.0906*distancei) + (-0.0146*math.pow(distancei,2)) + (4.5538*math.pow(10,-5)*math.pow(distancei,3))
                            dist_m0=-0.2615+0.0594*dist_m1-0.0007*math.pow(dist_m1,2)
                            dist_m = round(dist_m0,2)
                            # if dist_m <= 120:
                            #     dist_m = dist_m*100/167
                            #     plot_one_box(xyxy, im0, label=f'{dist_m:.1f}cm', color=colors(c, True), line_thickness=2)
                            #     print('dist<120',dist_m)
                            plot_one_box(xyxy, im0, label=f'{dist_m:.1f}cm', color=colors(c, True), line_thickness=2)
...
#                            avg_height2=int(avg_height[0])*2                                 # height average
                            ...
                            print('distancei',distancei,'dist_m',{dist_m}, 'time', datetime.utcnow().strftime('%S.%f')[:-3], 'width',width,'height',height)
