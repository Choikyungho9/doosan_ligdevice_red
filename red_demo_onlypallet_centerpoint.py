                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if c == 1:
                            ...
                            distancei = (2 * 31.4 * 180) * 5 / (width + height)  
                            dist_m=int(distancei*10)/10
                            plot_one_box(xyxy, im0*1.1, label=f'{distancei:.1f}cm', color=colors(c, True), line_thickness=2)
                            ...
                            x01_h = int(x_top + width * 0.295) # only use pallet boundary
                            y10_h = int(y_top + height * 0.53) # only use pallet boundary
                            x01_h2 = int(x_top + width * 0.705) # only use pallet boundary
